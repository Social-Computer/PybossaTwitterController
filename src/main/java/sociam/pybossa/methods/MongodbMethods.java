...
public class MongodbMethods {
	...
	public static Boolean insertTaskIntoMongoDB(ObjectId project_id, String task_text, String media_url,
		...
		try {
			...
			// String targettedFormat = MongoDBformatter.format(publishedAt);
			logger.debug("Inserting a task into MongoDB");
			if (pushTaskToMongoDB(publishedAt, project_id, task_status, task_text, media_url, task_type)) {
				return true;
			} else {
				return false;
			}
	...
	public static boolean pushTaskToMongoDB(String publishedAt, ObjectId project_id, String task_status,
				...
				if (iterable.first() == null) {
					database.getCollection(Config.taskCollection)
							.insertOne(new Document().append("publishedAt", publishedAt)
									.append("project_id", project_id).append("task_status", task_status)
